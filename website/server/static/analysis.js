...
"Model 1": [{type: "img", pic: "model_1_vars.png", text: `Even though expressed above our starting model comes from a trusted, knowledgable source
...
{type: "img", pic: "model_2_comparison.png", text: `Thanks to adding this new variable, there is a dramtic visual difference in the new model's boxplot and its evaluative measures.
...
"Marrying Markets and Predictions": [`The core of this project is to test my different model permutations to create the five predictive variables described in model 2, feed these projected variables into the scoring model and compare these scoring projections to the markets in order to make a decision on which side of the market to place a bet on.
...
Moreover, these graded decision are then passed to a new matrix that contains the odds of the model's respective decisions and multiplied by the amount bet on each game by the associated odds for that market decision.`,
...
Moving to the charts, the collected histograms shows the distribution of the team's and its opponent's four factors, the key variables in the scoring model. These visual instruments provide a look into backend of the model that ultimately influenced the crucial decision point.
Below, the cumulative winnings generated by the team's games are plotted versus time. I chose to show cumulative winnings as a representitive of the model's performance because the individual game winnings data points do not represented the compounding effect of placing bets. A win and a loss in terms of winnings are similarily equivalent and make for a noisey graph. The cumulative winnings, however, allows the bettor to observe the healthy of her bankroll over the course of the season.
It keeps the bettor informed on the long-term results on placing bets on this team's by each market and as a whole. Finally, the individual game data points that make up the team analysis are presented in the bottom table. The table contains each team's model projected score, market projected score and actual score for each game. The table provides an brief overview on the different valuations of the game and the true result as well as a link to the game's page which expands on each of these numbers and provides all the data considered for each decision point.
...
The game page gives the application user a through analyis of all the important numbers factoring into my decision for the game. It combines the model and market data with the actual results to allow for a more detailed analysis of the market and the model.
...
The final visualization gives the odds of the sportsbook and the winnings of my model's decision based off the market's number and odds. Like the table rows, the circles housing the odds and winnings are colored based on the partiuclar pick's grade.`],

"Future Plans": [{type: "li", text:`Player Based Modeling`, inner_text: "ddd"}, {type: "li", text: "Enhanced Filtering", inner_text: "ddd"}, {type: "li", text: "Bayesian Linear Regression"}, {type: "li", text: "Predictive Analysis on Current Games",  inner_text: "ddd"}, {type: "li", text: "Collecting and Analyzing Live Data",  inner_text: "ddd"}],
